...
fn main() -> ! {
    ...
    if let Some(dp) = stm32f401::Peripherals::take() {
        ...
        loop {
            ...
            hprintln!("Button is on!").unwrap();
            delay(10_000000); // Delay for at least n instruction cycles
            gpioc.odr.write(|w| w.odr13().set_bit()); // OFF
            hprintln!("Button is off!").unwrap();
            delay(10_000000); // Delay for at least n instruction cycles
        }
    };

    heprintln!("There was probably an error if you are seeing this!").unwrap();
    panic!();
}
